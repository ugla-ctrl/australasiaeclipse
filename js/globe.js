...
function init() {
  ...
  dom.addEventListener('pointermove', e => {
    if (dragging) {
      vx = (e.clientX - px) * 0.0021; vy = (e.clientY - py) * 0.0021; // lower drag gain — less twitchy
      rotY += vx; rotX += vy;
  ...
  dom.addEventListener('wheel', e => {
    e.preventDefault();
    dist = Math.max(1.45, Math.min(5, dist + e.deltaY * 0.0008)); // gentler zoom
    idle = 0;
  }, { passive: false });
...
function loop(t) {
  ...
  if (!dragging) {
    rotY += vx; rotX += vy;
    vx *= 0.80; vy *= 0.80; // quicker settle — less floaty spin after release
    idle += dt;
